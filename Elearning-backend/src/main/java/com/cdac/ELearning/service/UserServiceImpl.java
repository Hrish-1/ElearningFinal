	...
	@Override
	 public AuthenticationResponse login(Login loginRequest) throws InvalidKeyException {
		 ...
		 Authentication authenticate = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(loginRequest.getEmailId(),
	       ...
	       //String authenticationToken="dd";
	        ...
	        return new AuthenticationResponse(authenticationToken,userDeatails.getEmailId(),userDeatails.getFirstName(),userDeatails.getRole());
		 }
	 ...
	 public void addCourse(String emailId,String courseName) {
		System.out.println(courseName + " " + emailId);
		    ...
		    if(list.isEmpty()){
		    	Course c = this.courseRepository.findByCourseName(courseName);
		    	list.add(c);
		    	//user.getCourse().addAll(list);
	        ...
	        for(Course c :list)
			{
	        	...
	        	if(s1.equals(s2))
	        	{
					System.out.println(user.getEmailId()+ " inside if");
	        		return ;
	        	}
	        	else
	        	{
